...
void *producer(void *arg) {
	...
	while (1) {
		...
		// for the saftey in the following if-case
		pthread_mutex_lock(&so->lock);
		
		//if buffer is empty, place the line on the shared buffer
		while(so->full == 1) {
		...
		//if condition is false = buffer is not empty
		so->linenum = i;
...
void *consumer(void *arg) {
	...
	while (1) {
		...
		//if condition is false = buffer is full
		//Mutex lock is hold
		line = so->line;
		if (line == NULL) {
			pthread_cond_signal(&so->cv);
